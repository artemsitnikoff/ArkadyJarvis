...
async def lifespan(app: FastAPI):
    ...
    if settings.sales_report_bitrix_user_ids and settings.sales_report_recipients:
        # Дневной — только в будни (выходные продажник не работает)
        scheduler.add_job(
            sales_dept_summary_job,
            CronTrigger(
                day_of_week="mon-fri",
        ...
        # Недельный — каждую пятницу в 18:00 (тоже с транскрипцией, есть лимит max=25)
        scheduler.add_job(
            sales_dept_summary_job,
            CronTrigger(
            ...
            args=[bot, bitrix, ai_client, openrouter, 7, True],
        )
        logger.info(
