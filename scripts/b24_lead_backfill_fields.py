...
INDUSTRY_GROUPS: list[tuple[str, list[str]]] = [
    # Стоматология — отдельно, ДОЛЖНА быть раньше «Медицины» (первое совпадение
    # выигрывает, а «стом. клиника» матчит и стоматолог, и клиник).
    ("Стоматология", [
        "стоматолог", "ортодонт", "зубн прот", "имплант зуб",
    ]),
    ("Медицина", [
        "офтальм", "пластическ хирург", "клиник", "медиц", "ветерин",
        "психотерап", "массаж", "реабилитац", "наркологич", "гинеколог",
        "косметолог", "имплант",
    ]),
    # Риелторы — РАНЬШЕ «Застройщиков», т.к. «агентство недвижим» содержит
    # «недвижим» (которое тоже застройщики ловят при отсутствии порядка).
    ("Риелторы", [
        "риелтор", "риэлтор", "агентство недвижим", "бюро недвижим",
        "real estate agenc", "продаж квартир втор", "вторичн недвижим",
        "продаж готов квартир",
    ]),
    ("Застройщики", [
        "девелопмент", "застройщ", "новостройк", "новострой",
        "жилое строительств", "коттеджн", "коттедж посёл", "коттедж поселок",
        "загородн недвижим", "апартамент", "жилая недвижим", "жилое",
        "многоквартирн", "недвижим",
    ]),
    ...
    # Оборудование — РАНЬШЕ «B2B-сервиса»: «производство оборудования» матчит
    # «оборудован», должно идти в Оборудование, а не в Сервис.
    ("Промышленное оборудование", [
        "оборудован", "станк", "металлообраб", "горелоч", "котельн",
        "энергетическ", "сварочн", "приборостро", "промышленн оборудован",
        "ДГУ", "генератор",
    ]),
    ("B2B-сервис и инжиниринг", [
        "технические сервис", "инжиниринг", "b2b-инжиниринг", "промышленн",
        "ремонт оборудован", "монтаж оборудован", "обслуживан оборудован",
        "пусконаладочн", "шеф-монтаж", "инжен",
    ]),
    ("Телеком и IT", [
