...
**Что считает**:
- Набор проектов аудита = `dcj_projects` где `direction = "WEB - ПиК"` (≈288 из DCJ.xlsx) **плюс** `EXTRA_AUDIT_PROJECTS` (`hudson_analyzer.py`) — проекты по ключу из других направлений, которые тоже считаем. Сейчас там `COZYHOME` («CozyHome.ru Marketing», в DCJ.xlsx под «Маркетинг») и `MZNN` (внешний, в dcj_projects его нет — берётся `is_internal=0` из значения override). Override применяется на лету при загрузке → переживает пере-импорт DCJ.xlsx. Хочешь добавить проект — допиши `ключ: 0|1` (0 внешний / 1 внутренний) туда. На будущее: `DCNEW` — внутренний (`1`), пока не включён.
