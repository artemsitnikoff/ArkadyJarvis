...
- **План/факт за календарный месяц**: `month_won_sum` + `monthly_plan` (env `SALES_REPORT_MONTHLY_PLAN`, default 220000₽). WON считается через `STAGE_SEMANTIC_ID=S` + `>=CLOSEDATE start_of_month` **+ `cat_filter`** — счета 1С в план НЕ идут. **`month_won_deals`** — расшифровка факта: per-deal `{id, title, amount, company, closedate}`, где `company` = юрлицо (`COMPANY_ID` через `crm.company.get`) или контакт (fallback на `CONTACT_ID`). Промпт `sales_summary.md` печатает эти сделки под строкой «Факт», чтобы было видно, из чего цифра.
...
**Что считает**:
- Набор проектов аудита = `dcj_projects` где `direction = "WEB - ПиК"` (≈288 из DCJ.xlsx) **плюс** `EXTRA_AUDIT_PROJECTS` (`hudson_analyzer.py`) — проекты по ключу из других направлений, которые тоже считаем. `EXTRA_AUDIT_PROJECTS` — **авторитетный оверлей**: его `is_internal` **перебивает** значение из `dcj_projects` (`_load_web_pik_projects` делает `result.update(...)`) → классификация зафиксирована в коде и переживает пере-импорт DCJ.xlsx. Сейчас там: `COZYHOME`/`MZNN` (внешние), `DCNEW` (внутренний, АУП); `DA`/`HRD`/`SHR` (внутренние, АУП — DC Admin / HR Department / Стратегия и развитие HR); `TSAI`/`TSTM`/`TSS`/`EA` (внешние; `TSAI`/`EA` уже WEB-ПиК, перечислены явно для фиксации); `DFSH`/`VMP` (внешние). Хочешь добавить/переклассифицировать проект — допиши `ключ: 0|1` (0 внешний / 1 внутренний) туда (DB-сид не нужен, читается на лету). Список кандидатов на добавление приходит в `unknown_projects_<period>.md` (см. ниже) — не надо искать новые проекты руками.
...
- Норма по ТК РФ (per-dev, считается в `build_reports`):
  - База `WEEKLY_HOURS_NORM=32h` (полная ставка), `HOURS_PER_DAY=8h`. Минус `HOURS_PER_DAY` за каждый рабочий (Пн-Пт) праздник из производственного календаря РФ (`services/holidays_api.py` — `isdayoff.ru`, кэш в памяти).
  - **Неполная ставка** — `PART_TIME_NORMS: dict[developer_pattern → (норма, часов-в-дне)]` в `hudson_analyzer.py`. Сейчас `Осицын → (25.0, 5.0)`: 25h/нед, день праздника/отлучки вычитает 5h (а не 8h). Полная ставка = `(32, 8)`, числа full-time не меняются. Ключ = `developer_pattern` (стабилен при смене менеджера), читается на лету — DB-сид не нужен.
  - Минус «вес дня» (`per_day`) за каждый рабочий день отпуска/больничного/командировки (`bitrix.get_absences()` — `absence.list`, fallback на `calendar.event.get` по ключевым словам когда HR-модуль не установлен; флаг `_ABSENCE_LIST_DEAD` кэширует 404).
  - `weekly_norm <= 0` → `on_leave=True` (всю неделю пропустил, не оцениваем).
  - `compute_weekly_norm()` остался как хелпер «норма полной ставки за неделю» (build_reports его больше не зовёт — считает праздники напрямую через `count_holidays_in_workweek`).
...
- Всем — **4 .md аттачмента** (+ 5-й опционально): `bad_comments_<period>.md` (все плохие комменты с кликабельными Jira-ссылками `[PQ-918](jira.dclouds.ru/browse/PQ-918)`), `internal_hours_<period>.md` (внутренние часы по задачам, сгруппированы per-dev), `downtime_<period>.md` (простой по разработчикам — часы + issue + коммент, для приёмки менеджером) и `all_worklogs_<period>.md` (**ВСЕ** worklog'и каждого разработчика для сверки — каждая запись отдельной строкой: дата, issue-ссылка, часы, коммент; разбито на «✅ Учтено» / «💤 Простой» / «⚠️ Не учтено (проект вне аудита)» с ключом проекта — чтобы человек видел, почему его лог не зачёлся). Полные данные в .md, в Telegram только сводка чтобы не упереться в лимит 4096 байт (`TG_MAX=3800` + `_split_block` режет по строкам).
  - **`unknown_projects_<period>.md`** — 5-й аттачмент, шлётся **только если есть неизвестные проекты**. Агрегирует все out-of-scope записи по `project_key` (`_collect_unknown_projects` → `_build_unknown_projects_md`): ключ + название проекта (из `WorklogEntry.project_name`, поле тянется из Jira `fields.project.name`) + часы + разработчики + пример задачи + готовая строка `"KEY": 0,  # name` для копипаста в `EXTRA_AUDIT_PROJECTS`. Плюс общий блок для вставки всех сразу. Отсортировано по убыванию часов. Задача — не разбирать новые проекты каждую неделю руками, а вносить их пачкой из готового списка.
...
- При клике — полный отчёт **в личку нажавшему** (не в общий чат). Сообщения + 4 .md (+ `unknown_projects` если есть).
